...
public class MovieDownManager : MonoBehaviour
{
    ...
    public void finish()//pulldata -> OnSuccess 이후에 호출됨
    {
        ...
        //기존 기기 버전과 검사
        if (currentVersion == localVersion && latestSiteId == siteId) { // 기존 버전과 같음. 따로 에셋 다운로드 필요 없이 바로 instantiate
    ...
    public void DownLoadEverything(List<MovieDOB> movieInfo)
    {

        for (int i = 0; i < jsonData.IdlePoster.Length; i++)
        {
            s3.DownloadPoster(jsonData.IdlePoster[i]);
        }
        // s3.DownloadImage(movieInfo[0].leftlogo);
        for (int i = 0; i < movieInfo.Count; i++)
        {
            //포스터 다운로드
            s3.DownloadPoster(movieInfo[i].MoviePoster);
            //스티커 다운로드 및 스티커 이름 string[] 에 저장
            DownloadStickers(movieInfo[i].FaceCenters);
            DownloadStickers(movieInfo[i].HandCenters);
            DownloadStickers(movieInfo[i].Foregrounds);
        }
    }
    ...
    void DownloadStickers(string[] stickerNames)
    {
        // 스티커 다운로드
        if (stickerNames == null) return;
        foreach (var name in stickerNames)
        {
            print("---다운중... sticker name: " + name);
            if (name == "")
                continue;
            s3.DownloadSticker(name);
        }
    }
