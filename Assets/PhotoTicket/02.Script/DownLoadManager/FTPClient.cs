...
class FTPClient : MonoBehaviour
{
    ...
    async public void DownloadAsset(string assetName,int type)
    {
        StartCoroutine(DownloadAssetProcessing(assetName, type));

        /*
        var assetName = assetName1;

        bool download = await FetchUsers(assetName);
