...
namespace Moments
{
	...
	public sealed class Recorder : MonoBehaviour
	{
		...
		public Queue<Texture2D> m_Frames;
		Texture2D m_RecycledTexture;
		RenderTexture m_ResizedFrame;
		RenderTexture m_FullFrame;
		...
		public void Setup(bool autoAspect, int width, int height, int fps, float bufferSize, int repeat, int quality)
		{
			...
			m_ReflectionUtils.ConstrainMin(x => x.m_gifWidth, width);

			if (!autoAspect)
				m_ReflectionUtils.ConstrainMin(x => x.m_gifHeight, height);
		...
		public void FlushMemory()
		{
			...
			if (m_RecycledTexture != null)
			{
				Flush(m_RecycledTexture);
			}

			if (m_ResizedFrame != null)
			{
				Flush(m_ResizedFrame);
			}
			...
			foreach (Texture2D rt in m_Frames)
				Flush(rt);
		...
		public void ShowVideo(UnityEngine.UI.RawImage rawImage, double totalSeconds)
		{
			while (totalSeconds > m_BufferSize)
				totalSeconds -= m_BufferSize;
			...
			float ratio = (float)totalSeconds / (float)m_BufferSize;

			//Debug.Log("Play Video" +  totalSeconds.ToString() + " " + m_BufferSize.ToString());

			int idx = Mathf.RoundToInt(m_Frames.Count * ratio);
			if (idx >= m_Frames.Count) idx = m_Frames.Count - 1;
			if (idx < 0) idx = 0;

			rawImage.texture = m_Frames.ElementAt(idx);
		}
		...
		public IEnumerator SavePhoto(UnityEngine.UI.RawImage rawImage, string filename)
		{
			yield return new WaitForEndOfFrame();

			//RenderTexture.active = m_FullFrame;
			//capture photo texture

			Texture2D photoTexture = new Texture2D(1080, 1440, TextureFormat.RGB24, false);
			photoTexture.hideFlags = HideFlags.None;
			photoTexture.wrapMode = TextureWrapMode.Clamp;
			photoTexture.filterMode = FilterMode.Bilinear;
			photoTexture.anisoLevel = 0;

			RenderTexture.active = m_FullFrame;
			//m_PhotoTexture.ReadPixels(new Rect(77, 539, 926, 1231), 0, 0);
			photoTexture.ReadPixels(new Rect(0, 0, 1080, 1440), 0, 0);
			photoTexture.Apply();
			...
			string filepath = SaveFolder + "/" + filename + ".jpg";
			System.IO.File.WriteAllBytes(filepath, photoTexture.EncodeToJPG());

			ComplexSceneBehavior.texture2Ds[ComplexSceneBehavior.photoCount - 1] = LoadPhoto(ComplexSceneBehavior.photoCount.ToString());
		}
		...
		public Texture2D LoadPhoto(string filename)
		{
			...
			Texture2D photoTexture = new Texture2D(2, 2);  // Set initial size (will be resized)

			// Load the image data into the texture
			if (!photoTexture.LoadImage(imageData))
			{
				Debug.LogError("Failed to load image data into texture.");
				return null;
			}

			// Return the loaded photo texture
			return photoTexture;
		}
		...
		void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			{
				float resizedRatio = m_gifWidth / 1080.0f;
				if (m_ResizedFrame == null)
				{
					m_ResizedFrame = new RenderTexture(Mathf.RoundToInt(1080 * resizedRatio),
					    Mathf.RoundToInt(1920 * resizedRatio), 0, RenderTextureFormat.ARGB32);
					m_ResizedFrame.wrapMode = TextureWrapMode.Clamp;
					m_ResizedFrame.filterMode = FilterMode.Bilinear;
					m_ResizedFrame.anisoLevel = 0;
				}

				if (m_FullFrame == null)
				{
					m_FullFrame = new RenderTexture(1080, 1920, 0, RenderTextureFormat.ARGB32);
					m_FullFrame.wrapMode = TextureWrapMode.Clamp;
					m_FullFrame.filterMode = FilterMode.Bilinear;
					m_FullFrame.anisoLevel = 0;
				}

				Graphics.Blit(source, m_ResizedFrame);

				// Limit the amount of frames stored in memory
				if (m_Frames.Count >= m_MaxFrameCount)
					m_RecycledTexture = m_Frames.Dequeue();

				m_Time -= m_TimePerFrame;

				// Frame data
				Texture2D rt = m_RecycledTexture;
				m_RecycledTexture = null;

				if (rt == null)
				{
					rt = new Texture2D(m_gifWidth, m_gifHeight, TextureFormat.RGB24, false);
				...
				RenderTexture.active = m_ResizedFrame;
				//rt.ReadPixels(new Rect(77 * resizedRatio, 539 * resizedRatio, m_gifWidth, m_gifHeight), 0, 0);
				rt.ReadPixels(new Rect(0 * resizedRatio, 0 * resizedRatio, m_gifWidth, m_gifHeight), 0, 0);
				rt.Apply();
				RenderTexture.active = null;

				m_Frames.Enqueue(rt);
				Graphics.Blit(source, m_FullFrame);
			}
		...
		public void ComputeHeight()
		{
			...
			float effectiveAspect = 1080.0f / 1440.0f;
			m_gifHeight = Mathf.RoundToInt(m_gifWidth / effectiveAspect);
		}
		...
		IEnumerator PreProcess(string filename)
		{
			...
			for (int i = m_Frames.Count / 2; i < m_Frames.Count; i += 2)
			{
				GifFrame frame = ToGifFrame(ScaleTexture(m_Frames.ElementAt(i), 300, 400));
				frames.Add(frame);
				yield return null;
			}
		...
		GifFrame ToGifFrame(Texture2D source)
		{
			return new GifFrame() { Width = source.width, Height = source.height, Data = source.GetPixels32() };
		}
		#endregion
	}
}
