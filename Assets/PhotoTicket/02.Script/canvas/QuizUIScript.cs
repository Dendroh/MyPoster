...
public class QuizUIScript : MonoBehaviour, UIScript
{
    ...
    IEnumerator setQuizCategory() {
        ...
        // 카테고리 영역 화면 구성
        if (array.Count > 4) {  // 카테고리가 4개보다 많은 경우
    ...
    IEnumerator setQuiz() {
        ...
        if (obj["done"].ToString() == null || obj["done"].ToString().Equals("")) {    // 완료 설정이 없는 경우
            errorPopup.SetActive(true);
            errorText.text = "완료 화면 설정을 해주세요.";
            yield break;
        }

        if (obj["file_path"].ToString() == null || obj["file_path"].ToString().Equals("")) {    // 파일 경로 없는 경우
            errorPopup.SetActive(true);
        ...
        for (int j = 0; j < array.Count; j++) {
            ...
            if (array[j]["file_list"] != null && array[j]["file_list"].Count() > 0) {    // 설정된 이미지 파일이 있으면 추가
                List<string> fileList = new List<string>();
                for (int i = 0; i < array[j]["file_list"].Count(); i++) {   // 파일 목록이 존재하는 경우
            ...
            if (array[j]["type"].ToString().Equals("0")) {  // O,X 퀴즈
                ...
                if (array[j]["answer"].ToString().Equals(ConstantsScript.WRONG_ANSWER)) {   // 정답이 X인 경우
                ...
                if (array[j]["item_list"].Count() > 0) {    // 보기 목록이 있는 경우
    ...
    public void clickCategory(int id) {
        ...
        for (int i = 0; i < quizList.Length; i++) {
            if (quizList[i].GetComponent<QuizController>().categoryId == id) {  // 퀴즈의 카테고리 id와 같은 경우
    ...
    public void clickAnswer(int quizIndex, string isAnswer, int answerIndex, string type, string answerCount, string ox) {
        ...
        if (type.Equals(ConstantsScript.OX)) { // OX퀴즈
            ...
            if (isAnswer.Equals(ConstantsScript.CORRECT_ANSWER)) { // 정답인 경우
            ...
            if (Convert.ToInt32(answerCount) == 1) {    // 정답이 1개인 경우
                // 정답 팝업 활성화
                quizList[quizIndex].Find("Answer Popup").gameObject.SetActive(true);
            ...
            } else if (Convert.ToInt32(answerCount) > 1) {   // 중복 정답인 경우
                chooseCount++;
                ...
                if (chooseCount == Convert.ToInt32(answerCount)) {
                    ...
                    if (correctCount == Convert.ToInt32(answerCount)) { // 정답인 경우
                        // O, X 정답 표시
    ...
    public void clickNext(int quizIndex) {
        ...
        if (completeCount < quizCount - 1) { // 다음 문제가 있는 경우
            // 다음 퀴즈 활성화
    ...
    IEnumerator addImages(List<string> filePathList, RectTransform rectTransform) {
        ...
        for (int i = 0; i < filePathList.Count(); i++) {
            using (WWW www = new WWW(filePathList[i])) {    // 서버로부터 이미지 정보 가져오기
                yield return www;
                if (string.IsNullOrEmpty(www.error)) {  // 성공
