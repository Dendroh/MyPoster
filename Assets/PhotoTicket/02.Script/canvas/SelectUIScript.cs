...
public class SelectUIScript : MonoBehaviour, UIScript
{
    ...
    public void checkPrint() {
        if (_netClient.status == true) {    // 정상 연결
            //큐 탐색 시작
        ...
        } else if (_netClient.status == false && _netClient.isRetry == true) {  // 재연결 시도중
            reconnectPopup.SetActive(true);
        } else if (_netClient.status == false && _netClient.isRetry == false) { // 재연결 실패
    ...
    public IEnumerator CheckQueue() {
        ...
        while (count < maxLoopCount) {
            count++;
            if (_netClient.status == true) {    // 정상 연결
                if (netClientErrorCanvas.activeSelf == true) {  // NetClientError 화면이 활성화된 경우
                ...
                if (checkPrintResult.Length > 0) {  // 프린터 체크 데이터가 있는 경우
                    // 스크립트 별 동작 구성
                    if (printStatus.Equals("disable")) {    // 프린터 상태 에러
                    ...
                    } else if (printStatus.Equals("proceed")) { // 진행중
                    ...
                    checkPrintResult = "";
                    yield break;
                } else if (paymentResult.Length > 0) {  // 결제데이터가 있는 경우
                    // 스크립트 별 동작 구성
                    if (paymentProcess) {   // 성공, 진행중
                ...
                if (canvas.Equals("payment")) { // 결제 화면인 경우
                    // UI 재구성
                    FlowController.instance.Loading(false);
                ...
                if (canvas.Equals("payment")) { // 결제 화면인 경우
                    ...
                    paymentUIScript.SetPayGuide(0);
                    paymentUIScript.SetLoadingGuide(4);
                } else if (canvas.Equals("agreement")) {    // 약관 동의 화면인 경우
                    // UI 재구성
                    FlowController.instance.Loading(false);
                    agreementUIScript.SetLoadingProgress(false);
                    agreementUIScript.SetLoadingGuide(3);
    ...
    private void ReceiveMessage(string message) {
        ...
        if (sendType.Equals("check_print")) {   // 프린터 상태 체크
            checkPrintResult = printResponse.Result;
            if (checkPrintResult.Equals("1")) { // 사용 가능
                printStatus = "able";
            } else if (checkPrintResult.Equals("2")) {  // 진행중
                printStatus = "proceed";
            } else {    // 불가능
                printStatus = "disable";
            }
