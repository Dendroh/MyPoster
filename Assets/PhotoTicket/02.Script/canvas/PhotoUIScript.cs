...
public class PhotoUIScript : MonoBehaviour, UIScript
{
    ...
    {
        //포스터 프리팹 설정
        if (MovieDownManager.completeDownload) {    // 포스터 다운로드가 완료된 경우
        ...
        if (movieScroll.value > 0.9f) {
            upArrow.SetActive(false);
        } else if (movieScroll.value < 0.1f) {
    ...
    public void Dispose()
    {
        ...
        resultLoadingShield.SetActive(true);
    ...
    IEnumerator SetPoster() {
        for (int i = 0; i < downManager.jsonData.movieInfo.Count; i++) {
            ...
            if (i == downManager.jsonData.movieInfo.Count - 1) {    // 마지막 반복인 경우
                doneSetPoster = true;
            }
    ...
    public void selectPoster(int movieNumber) {
        ...
        if (downManager.jsonData.movieInfo[movieNumber].isChromakey) {  // 크로마키 컨텐츠인 경우
            if (AutoBackgroundQuad.updateQuad == true) {    // Quad 설정이 완료된 경우
